...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Direct-four-view only: search safe global Gaussian colour, opacity, and scale calibration candidates "
...
def _search_direct_gaussian_appearance(
    model,
    opt,
    device,
    initial_gaussians,
    ...
    """Safely select an LGM appearance calibration without CUDA backward.

    LGM's optional rasterizer build is stable for inference on Kaggle T4 but
    its CUDA backward kernel can crash after a PyTorch/CUDA version change.
    This forward-only search is consequently intentional: calculate a global
    RGB affine correction from the four source views, try interpolations of
    it, then search modest global opacity/scale changes. It never modifies
    geometry and retains the lowest-LPIPS-MRC candidate.
    """
    ...
    source = initial_rendered.detach().float()
    mask = (target < 0.97).any(dim=1, keepdim=True).expand_as(target)
    x = source.permute(0, 2, 3, 1)[mask.permute(0, 2, 3, 1)].view(-1, 3)
    y = target.permute(0, 2, 3, 1)[mask.permute(0, 2, 3, 1)].view(-1, 3)
    if x.numel() == 0:
        raise RuntimeError("No foreground pixels found for safe appearance calibration.")
    x_mean = x.mean(dim=0)
    y_mean = y.mean(dim=0)
    gain = ((x - x_mean) * (y - y_mean)).mean(dim=0) / ((x - x_mean).square().mean(dim=0) + 1e-6)
    gain = gain.clamp(0.75, 1.25)
    bias = (y_mean - gain * x_mean).clamp(-0.10, 0.10)
    base = initial_gaussians.detach().float()
    best_gaussians = base.clone()
    best_rendered = initial_rendered.detach().float()
    best_mrc = initial_mrc
    history = [{"candidate": "baseline", "mrc": initial_mrc["mrc"], "accepted": True}]

    def evaluate(label: str, candidate):
        nonlocal best_gaussians, best_rendered, best_mrc
        with torch.no_grad():
            rendered = _render_views(model, opt, device, candidate, ANGLES, elevation).detach()
            mrc, _ = compute_mrc(target, rendered, ANGLES, metric=mrc_metric, lpips_model=lpips_model)
        improved = mrc["mrc"] < best_mrc["mrc"]
        if improved:
            best_gaussians = candidate.detach().clone()
            best_rendered = rendered.clone()
            best_mrc = mrc
        history.append({"candidate": label, "mrc": mrc["mrc"], "best_mrc": best_mrc["mrc"], "accepted": improved})
        marker = "new best" if improved else "kept best"
        print(f"[Appearance] {label}: MRC={mrc['mrc']:.6f}; best={best_mrc['mrc']:.6f} ({marker})", flush=True)

    print(f"[Appearance] baseline MRC={best_mrc['mrc']:.6f}; searching safe forward-only candidates...", flush=True)
    for strength in (0.25, 0.50, 0.75, 1.00):
        candidate = base.clone()
        adjusted_gain = 1 + strength * (gain - 1)
        adjusted_bias = strength * bias
        candidate[..., 11:] = (candidate[..., 11:] * adjusted_gain + adjusted_bias).clamp(0, 1)
        evaluate(f"rgb_affine_strength={strength:.2f}", candidate)

    colour_best = best_gaussians.clone()
    for opacity_factor, scale_factor in ((0.90, 1.00), (1.10, 1.00), (1.00, 0.90), (1.00, 1.10), (0.90, 0.90), (1.10, 1.10)):
        candidate = colour_best.clone()
        candidate[..., 3:4] = (candidate[..., 3:4] * opacity_factor).clamp(1e-4, 1 - 1e-4)
        candidate[..., 4:7] = (candidate[..., 4:7] * scale_factor).clamp_min(1e-5)
        evaluate(f"opacity={opacity_factor:.2f},scale={scale_factor:.2f}", candidate)

    return best_gaussians, best_rendered, best_mrc, {
        "enabled": True,
        "method": "forward-only global RGB affine plus opacity/scale candidate search",
        "rgb_affine_fit": {"gain": gain.detach().cpu().tolist(), "bias": bias.detach().cpu().tolist()},
        "history": history,
    }
...
def main() -> None:
    ...
    appearance_search = {"enabled": False}
    if args.appearance_search:
        print(
            f"Calibrating the selected direct reconstruction at {selected_elevation:+.1f}\N{DEGREE SIGN}...",
            flush=True,
        )
        gaussians, rendered_tensor, mrc, appearance_search = _search_direct_gaussian_appearance(
            model,
            opt,
            device,
            gaussians,
    ...
    metadata = {
        ...
        "direct_appearance_search": appearance_search,
